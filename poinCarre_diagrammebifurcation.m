...
% af=0.1; ne=30; da=(af-a)/(ne-1); ne=300;
ai=0.01; af=0.15; ne=200; da=(af-ai)/(ne-1); 
% ai=0.0217; af=0.0218; ne=10; da=(af-ai)/(ne-1); 
%
...
for k=1:ne
    ...
    xlabel('t (s)')
    ylabel('\theta (rd)')
    axis([(nt-nth*ni)*dt tf -0.25 0.25]);
...
%  
    ...
    xlabel('a (m)')
    ylabel('z (m)')
    axis([0 ne*da -0.6 0.6]);
    title('diagramme de bifircation "en z"')
    drawnow
    hold on
%
    figure(hfg(6));
    ...
    xlabel('a (m)')
    ylabel('\theta (rd)')
    axis([0 ne*da -0.04 0.04]);
    title('diagramme de bifircation "en theta"')
    drawnow
    hold on
%
end
